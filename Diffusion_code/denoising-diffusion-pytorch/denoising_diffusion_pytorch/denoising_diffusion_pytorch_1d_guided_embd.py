...
class Unet1D(nn.Module):
    def __init__(
        ...
        # self.null_classes_emb = nn.Parameter(torch.randn(1920))
        self.null_classes_emb = nn.Parameter(torch.randn(640))

        classes_dim = dim * 4

        self.classes_mlp = nn.Sequential(
            # nn.Linear(1920, classes_dim), # condition on source weights
            nn.Linear(640, classes_dim),
            nn.GELU(),
...
class GaussianDiffusion1D(nn.Module):
    ...
    def sample(self, classes, cond_scale = 6., rescaled_phi = 0.7):
        batch_size, seq_len, channels = classes.shape[0], self.seq_length, self.channels
        sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample
        return sample_fn(classes, (batch_size, channels, seq_len), cond_scale, rescaled_phi)
...
import pandas as pd
sys.path.append(os.path.expanduser("~/Projects/GenAIProject/"))
sys.path.append(os.path.expanduser("~/Projects/GenAIProject/finetuning"))
...
class Trainer1D(object):
    ...
    def train(self):
        ...
        train_loss = []
        accs = {corr : [] for corr in self.corruptions}
        with tqdm(initial = self.step, total = self.train_num_steps, disable = not accelerator.is_main_process) as pbar:
            ...
            while self.step < self.train_num_steps:
                if (self.step % 500) == 0:
                    ...
                    with torch.no_grad():
                        for corr_i,corr in enumerate(self.corruptions):
                            test_model = load_model(model_name='Standard',dataset='cifar10',threat_model='corruptions').eval().cuda()

                            if corr == 'clean':
                                x_clean, y_clean = load_cifar10(500)
                                test_ds = TensorDataset(x_clean,y_clean)
                            else:                   
                                # get data for specific corruption
                                train_ds, val_ds, test_ds = get_cifar10c_data([self.corruptions[corr_i]],1000)
                                test_ds = Subset(test_ds,torch.arange(500))
                            ...
                            for batch_idx, (data,target) in enumerate(tqdm(dl_t)):
                                ...
                                generated_params.append(self.model.sample(input_activations, cond_scale=3.))
                            ...
                            for batch_idx, (data,target) in enumerate(tqdm(dl_t)):
                                ...
                                test_model = put_back_params(test_model,params)
                            ...
                            print(f"================ Evaluate: Acc for {corr} = {acc}")
                            accs[corr].append(acc.item())

                total_loss = 0.

                for _ in range(self.gradient_accumulate_every):
                    data, embds = next(self.dl)
                    data, embds = data.to(device), embds.to(device)
                    # cat_params = copy.deepcopy(og_params)
                    # cat_params = cat_params.unsqueeze(0).repeat(embds.shape[0], 1)
                    # embds = torch.cat([embds,cat_params.cuda()],dim=1)
                    

                    with self.accelerator.autocast():
                        loss = self.model(data, classes = embds)
                ...
                if (self.step % 500) == 0:
                    ...
                    results_path = os.path.expanduser(f"~/Projects/GenAIProject/saved_data/cond_diff_results_embd/")
                    Path(results_path).mkdir(parents=True, exist_ok=True)
                    
                    df = pd.DataFrame(accs)
                    df.to_csv(os.path.join(results_path,"acc"))

                #     plt.plot(np.arange(len(train_loss)),train_loss,label='train_loss')
                #     plt.plot(np.arange(len(train_loss)),accs,label='accuracy')
                #     plt.legend()
                #     plt.grid()
                #     plt.ylabel("A")
                #     plt.xlabel('Training Iteration')
                #     plt.savefig(os.path.expanduser("~/Projects/GenAIProject/plt.png"))
                #     plt.clf()
