            ...
            while self.step < self.train_num_steps:
                ...
                if (self.step % 500) == 0:
                    ...
                    with torch.no_grad():
                        ...
                        corr_classes = torch.arange(len(self.corruptions)).cuda() # sample 5 classes
                        sampled_seq = self.model.sample(corr_classes, cond_scale=3.)
                        # for each class
                        for seq_i, seq in enumerate(sampled_seq):
                            test_model = put_back_params(test_model,seq)
                            test_model = test_model.eval()
                            # get data for specific corruption
                            train_ds, val_ds, test_ds = get_cifar10c_data([self.corruptions[seq_i]],1000)
                            dl_t = DataLoader(test_ds,batch_size=256)
                            criterion = nn.CrossEntropyLoss()

                            acc,loss = validate(test_model,dl_t,'cuda',criterion)
                            print(f"================ Evaluate: Acc for {self.corruptions[seq_i]} = {acc}")
                            accs[self.corruptions[seq_i]].append(acc)
                ...
                if (self.step % 500) == 0:
                    ...
                    results_path = os.path.expanduser(f"~/Projects/GenAIProject/saved_data/cond_diff_results/")
